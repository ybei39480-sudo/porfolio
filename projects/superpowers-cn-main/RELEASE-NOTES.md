...
子代理审查循环 (分派新代理审查计划/规范) 使执行时间翻倍 (~25 min overhead) 在没有明显提高计划质量的情况下. 跨 5 个版本各 5 次试验的回归测试 showed 相同的质量分数 无论审查循环是否运行.

- **brainstorming** — 取代规范审查循环 (子代理分派 + 3 次迭代上限) 使用内联规范自审检查清单: 占位符扫描, 内部一致性, 范围检查, 模糊性检查
- **writing-plans** — replaced Plan Review Loop (子代理分派 + 3 次迭代上限) with inline Self-Review checklist: spec coverage, 占位符扫描, type consistency
...
- 自审每次运行捕获 3-5 个真实 bug in ~30s 而不是约 25 分钟, 缺陷率相当 to the subagent approach

### 头脑风暴服务器

- **会话目录重构** — the brainstorm server session directory now contains 两个同级子目录: `content/` (提供给浏览器的 HTML 文件) and `state/` (事件、服务器信息、pid、日志). 之前，服务器状态和用户交互数据与提供的内容存储在一起, 使它们可通过 HTTP 访问. The `screen_dir` and `state_dir` paths are both included in the server-started JSON. (Reported by 吉田仁)

### Bug 修复

- **Owner-PID 生命周期修复** — the brainstorm server's owner-PID 监控有两个 bug 导致 60 秒内错误关闭: (1) 跨用户 PIDs 的 EPERM (Tailscale SSH, etc.) was 被视为 "process dead", and (2) 在 WSL 上祖父 PID 解析为 一个在第一次生命周期检查之前退出的短命子进程 the first lifecycle check. 通过将 EPERM 视为 "alive" and 在启动时验证 owner PID — if it's already dead, monitoring is disabled and 服务器依赖 30 分钟空闲超时. 这也移除了 Windows/MSYS2 特定的 carve-out from `start-server.sh` 因为服务器现在通用处理它. (#879)
...
- **codex-tools** — 添加了命名代理分派映射 记录如何转换 Claude Code's 命名代理类型 to Codex's `spawn_agent` with worker roles (PR #647 by @arittr)
- **codex-tools** — 添加了环境检测和 Codex App 完成部分 用于感知 worktree 的技能 (by @arittr)
- **设计规范** — 添加了 Codex App 兼容性设计规范 (PRI-823) 涵盖只读环境检测, worktree 安全的技能行为, and 沙盒回退模式 (by @arittr)
...
显著减少 token 使用 并加快规范和计划审查 通过消除不必要的审查轮次 并收紧审查者焦点.

- **单一整体计划审查** — 计划审查者现在一次审查完整计划 而不是逐块. 移除所有块相关概念 (`## 块 N:` 标题、1000 行块限制、逐块分派).
- **提高阻止问题的门槛** — 规范和计划审查者提示 现在包含 "Calibration" section: 只标记实施过程中会导致真正问题的事项. 措辞、风格偏好和格式小问题不应阻止批准.
- **减少最大审查迭代次数** — 从 5 次到 3 次 对于规范和计划审查循环. 如果审查者校准正确, 3 轮就够了.
- **精简审查者检查清单** — 规范审查者从 7 个类别精简到 5 个; 计划审查者从 7 个到 4 个. 移除格式-focused 检查 (任务语法、块大小) 改为关注实质 (可构建性、规范对齐).

### OpenCode

- **单行插件安装** — OpenCode 插件现在自动注册 技能目录通过 `config` hook. 不需要符号链接或 `skills.paths` config needed. 安装只需添加一行到 `opencode.json`. (PR #753)
- **添加 `package.json`** 因此 OpenCode 可以将 superpowers 作为 npm 包从 git 安装.
...
- **Cursor hooks** — added `hooks/hooks-cursor.json` with Cursor's camelCase format (`sessionStart`, `version: 1`) and updated `.cursor-plugin/plugin.json` to reference it. 修复 platform detection in `session-start` to check `CURSOR_PLUGIN_ROOT` first (Cursor may also set `CLAUDE_PLUGIN_ROOT`). (Based on PR #709)
...
- 添加 step 7 (spec review loop) to the checklist and corresponding nodes to the dot graph
...
- 修复 Cursor install command in README: `/plugin-add` → `/add-plugin` (confirmed via Cursor 2.5 release announcement)

**User review gate in brainstorming** (#565)

- 添加 explicit user review step between spec completion and writing-plans handoff
...
- 设计规范 and implementation plan in `docs/superpowers/`
...
添加 explicit priority ordering to using-superpowers:
...
添加 `<SUBAGENT-STOP>` block to using-superpowers. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
添加 an `EnterPlanMode` intercept to the skill flow graph. When the model is about to enter Claude's native plan mode, it checks whether brainstorming has happened and routes through the brainstorming skill instead. Plan mode is never entered.

### 修复

**SessionStart hook now runs synchronously**

更改 `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-superpowers instructions weren't in context for the first message.
...
**Windows: 修复 Claude Code 2.1.x hook execution (#331)**
...
**Windows: 修复 O(n^2) `escape_for_json` performance**
...
**Codex: 修复 Windows/PowerShell invocation (#285, #243)**
...
- 修复 `~/` path expansion on Windows — PowerShell doesn't expand `~` when passed as an argument to `node`. 更改 to `$HOME` which expands correctly in both bash and PowerShell.

**Codex: 修复 path resolution in installer**
...
**Codex: 修复 stale skills path in writing-skills**
...
添加 `using-git-worktrees` as a required skill for both `subagent-driven-development` and `executing-plans`. Implementation workflows now explicitly require setting up an isolated worktree before starting work, preventing accidental work directly on main.
...
- 添加 worktree requirement test for subagent-driven-development
- 添加 main branch red flag warning test
- 修复 case sensitivity in skill recognition test assertions
...
**OpenCode: 修复 symlink instructions (#339, #342)**

- 添加 explicit `rm` before `ln -s` (fixes "file already exists" errors on reinstall)
- 添加 missing skills symlink step that was absent from INSTALL.md
...
**OpenCode: 修复 agent reset on session start (#226)**

The previous bootstrap injection method using `session.prompt({ noReply: true })` caused OpenCode to reset the selected agent to "build" on first message. Now uses `experimental.chat.system.transform` hook which modifies the system prompt directly without side effects.

**OpenCode: 修复 Windows installation (#232)**

- Removed dependency on `skills-core.js` (eliminates broken relative imports when file is copied instead of symlinked)
- 添加 comprehensive Windows installation docs for cmd.exe, PowerShell, and Git Bash
- Documented proper symlink vs junction usage for each platform

**Claude Code: 修复 Windows hook execution for Claude Code 2.1.x**
...
- 添加 "BEFORE any response or action" - the original wording only mentioned "response" but Claude would sometimes take action without responding first
- 添加 reassurance that invoking a wrong skill is okay - reduces hesitation
- 添加 new red flag: "I know what that means" → Knowing the concept ≠ using the skill

**添加 explicit skill request tests**
...
添加 `disable-model-invocation: true` to all three slash commands (`/brainstorm`, `/execute-plan`, `/write-plan`). Claude can no longer invoke these commands via the Skill tool—they're restricted to manual user invocation only.
...
修复 a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-superpowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- 添加 "How to Access Skills" section to `using-superpowers`
- 更改 "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `superpowers:brainstorming`)

**添加 GitHub thread reply guidance to receiving-code-review** (h/t @ralphbean)

添加 a note about replying to inline review comments in the original thread rather than as top-level PR comments.

**添加 automation-over-documentation guidance to writing-skills** (h/t @EthanJStark)

添加 guidance that mechanical constraints should be automated, not documented—save skills for judgment calls.
...
- **Linux Compatibility**: 修复 polyglot hook wrapper (`run-hook.cmd`) to use POSIX-compliant syntax
...
- **Improved Documentation**: Rewrote README to explain problem/solution clearly
  ...
  - 添加 complete workflow description (brainstorm → plan → execute → finish)
...
- 添加 unified `superpowers-codex` script with bootstrap/use-skill/find-skills commands
...
- 更改 skill invocation instructions from Read tool to Skill tool
...
### Files 更改
- Updated: `skills/using-superpowers/SKILL.md` - 更改 tool references from Read to Skill
...
- 添加 EXTREMELY-IMPORTANT block with absolute language about mandatory skill checking
...
- 添加 Common Rationalizations section with 8 specific evasion patterns
...
### Files 更改
- Updated: `skills/using-superpowers/SKILL.md` - 添加 three layers of enforcement to prevent skill-skipping rationalization
...
- 添加 `superpowers:code-reviewer` agent to plugin's `agents/` directory
...
- 添加 Phase 4: Design Documentation to brainstorming skill
...
- **修复 command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/superpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
- 添加 Quick Reference table showing phases, activities, and tool usage
- 添加 copyable workflow checklist for tracking progress
- 添加 decision flowchart for when to revisit earlier phases
- 添加 comprehensive AskUserQuestion tool guidance with concrete examples
- 添加 "Question Patterns" section explaining when to use structured vs open-ended questions
- Restructured Key Principles as scannable table

**Anthropic best practices integration**
- 添加 `skills/writing-skills/anthropic-best-practices.md` - Official Anthropic skill authoring guide
...
- 修复 description grammar and voice (fully third-person)
- 添加 Quick Reference tables for scanning
- 添加 workflow checklists Claude can copy and track
...
- 修复 `defense-in-depth` name mismatch (was `Defense-in-Depth-Validation`)
- 修复 `receiving-code-review` name mismatch (was `Code-Review-Reception`)
- 修复 `commands/brainstorm.md` reference to correct skill name
...
- 添加 reference to Anthropic's official best practices
...
- **修复 false warning when local skills repo is ahead of upstream** - The initialization script was incorrectly warning "New skills available from upstream" when the local repository had commits ahead of upstream. The logic now correctly distinguishes between three git states: local behind (should update), local ahead (no warning), and diverged (should warn).

## v2.0.1 (2025-10-12)

### Bug 修复

- **修复 session-start hook execution in plugin context** (#8, PR #9) - The hook was failing silently with "Plugin hook error" preventing skills context from loading. 修复 by:
...
- 添加 "Why" explanations for all workflows
...
- 添加 token efficiency section (word count targets)
...
- 修复 duplicate upstream remote addition when forking
- 修复 find-skills double "skills/" prefix in output
- Removed obsolete setup-personal-superpowers call from session-start
- 修复 path references throughout hooks and commands
...
- 修复 skill names and references
- Updated Meta skills list

### Testing Documentation
- 添加 comprehensive testing checklist (`docs/TESTING-CHECKLIST.md`)
